...
user_states = {}
# NEW: Global flag to control the auto-allocator
...
def init_db():
    ...
    # MODIFIED: Add columns to store seated customer info directly on the table.
    # This prevents data loss when a user is removed from the waiting queue.
    try:
...
# MODIFIED: Clears new customer columns when a table is freed.
...
# MODIFIED: Now seats a customer by updating table info and removing them from the queue.
    ...
    try:
        ...
        conn.execute("""
            UPDATE tables SET 
                ...
                people_count = ?
            WHERE id = ?
        """, (customer_id, datetime.datetime.now(), customer_name, people_count, table_id))
...
# MODIFIED: Checks the global flag before running.
def attempt_seating_allocation():
    ...
    # Prioritize seating the longest-waiting customer who fits *any* table.
    for customer in waiting_customers:
        if customer['id'] in seated_customer_ids:
            continue

        # Find the best fitting table for this customer (smallest capacity that fits)
        best_fit_table = None
        min_waste = float('inf')
        ...
        # If a suitable table was found, seat the customer and mark resources as used
        if best_fit_table:
            seat_customer(
                customer['id'],
                best_fit_table['id'],
            ...
            # After seating one, restart the process to re-evaluate priorities
            # This makes the allocation fairer for the next person in the queue.
            attempt_seating_allocation()
            return # End this run, as a recursive call has started a new one.
...
def get_occupied_tables():
    ...
    conn = get_db_connection()
    rows = conn.execute("""
        SELECT table_number, occupied_timestamp, customer_name, people_count
        FROM tables 
    ...
    # Manually convert timestamp strings to datetime objects
    occupied = []
    for row in rows:
        row_dict = dict(row) # Convert the database row to a mutable dictionary
        if row_dict['occupied_timestamp'] and isinstance(row_dict['occupied_timestamp'], str):
            try:
                # The format must match how it's stored in SQLite
                # Added support for different fractional second precisions
                if '.' in row_dict['occupied_timestamp']:
                 ...
                 # if parsing fails, set to None or a default to avoid crashing
...
# MODIFIED: Now also fetches people_count for manual seating.
...
# MODIFIED: This is the main route for the dashboard. It now passes more data.
@app.route('/admin')
def admin_dashboard():
    ...
    return render_template('admin.html',
        ...
        free_tables=get_free_tables(),
        occupied_tables=get_occupied_tables(), # Pass occupied tables
        auto_allocator_status='ON' if AUTO_ALLOCATOR_ENABLED else 'OFF' # Pass allocator status
    )

# NEW: Route to handle the auto-allocator toggle from the dashboard.
...
def toggle_auto_allocator():
    ...
    # If it was just turned ON, run an allocation check immediately
...
def free_table():
    ...
    if update_table_status_to_free(request.form.get('table_number')):
        # After freeing a table, try to seat the next person in the queue
        attempt_seating_allocation()
    return redirect(url_for('admin_dashboard'))
    ...
    if customer_id and table_id:
        ...
        if customer and table:
            seat_customer(
                ...
                customer['phone_number'], 
                customer['name'],
                customer['people_count'] # Pass people_count
            )
    return redirect(url_for('admin_dashboard'))
